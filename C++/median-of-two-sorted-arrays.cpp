...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
    	...
    	while(from <= to) {
//			printf("findMedianSortedArrays1, %d, %d\n", from, to);
    		i = (from + to)/2;
    		j = halfLen - i;
//			printf("findMedianSortedArrays2, %d, %d\n", i, j);
			if(i > 0 && j < nums2.size() && nums1[i-1] > nums2[j] || j < 0) {
				to = i - 1;
			}
			else if(j > 0 && i < nums1.size() && nums2[j-1] > nums1[i] || j > nums2.size()) {
				from = i + 1;
			}
			else {
				if(i == 0 && j == 0) {
...
//		printf("findMedianSortedArrays3, %d, %d\n", maxLeft, minRight);
		double mid = (s1 + s2)%2 == 0 ? (maxLeft + minRight)/2.0 : minRight;		
        return mid;
    }
};
